...
def test_chat_functionality():
    ...
    try:
        ...
        if register_response.status_code != 200 and register_response.status_code != 409:  # 409 means user already exists
    ...
    try:
        ...
        if login_response.status_code == 200:
            ...
            if "already" in login_response.text or login_response.status_code == 409:
                login_response = requests.post(f"{base_url}/auth/signin", json=login_data)
                if login_response.status_code == 200:
                    token_data = login_response.json()
                    jwt_token = token_data.get("access_token")
                    user_id = token_data["user"]["id"]
                    print(f"[OK] Got JWT token after retry, user_id: {user_id}")
                else:
                    print(f"[ERROR] Login still failed after retry: {login_response.text}")
                    return False
            else:
                return False
    except Exception as e:
